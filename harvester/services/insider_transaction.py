...
class InsiderTransactionService:
    ...
    async def fetch_and_publish(self, request: FetchInsiderTransactionsRequest) -> None:
        ...
        total_published = 0
        run_id = str(request.correlation_id) if request.correlation_id else str(uuid.uuid4())
        part_num = 0
        ...
        for transactions_batch in self._provider.fetch_transactions(request.max_filing_date):
            ...
            try:
                ...
                logger.info("Published DataFileReadyEvent for %d insider transactions (Run ID: %s, Part: %d)", meta["count"], run_id, part_num)
