"""Fetch + publish derived financial metrics."""
...
class DerivedService:
    """Streams derived metrics from SimFin source statements to Parquet."""

    def __init__(self, provider: SimFinProvider) -> None:
        self._provider = provider

    async def fetch_and_publish(self, request: FetchDerivedRequest) -> None:
        ...
        for template in request.templates:
            ...
            await publishers.data_file_ready.publish(event, key=f"derived:{run_id}")

            logger.info(
                "Published %d derived rows for template '%s' to Parquet [cid=%s]",
                len(rows),
                template,
