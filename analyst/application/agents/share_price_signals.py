...
_PROMPT_PATH = Path(__file__).parent / "prompts" / "share_price_signals.md"


class SharePriceSignalsOutput(BaseModel):
    """The structured output produced by the SharePriceSignalsAgent."""
...
class SharePriceSignalsAgent(Agent[SharePriceSignalsOutput]):
    """
    Agent responsible for analyzing daily market signals and historical monthly signals.
    """

    name = "SharePriceSignals"
    output_model = SharePriceSignalsOutput

    def __init__(self) -> None:
        self.system_prompt = _PROMPT_PATH.read_text(encoding="utf-8")

    async def run(self, ctx: AgentContext) -> AgentOutput[SharePriceSignalsOutput]:
